                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="bingliId" name="bingliId" type="hidden">
                                    ...
                                    <div class="form-group  col-md-6">
                                        <label>医嘱详情</label>
                                        <input id="yizhuContentupload" name="file" type="file">
...
<script>
    ...
    function upload() {
        ...
        $('#yizhuContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('yizhuContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.yizhuContent != null && ruleForm.yizhuContent != 'null' && ruleForm.yizhuContent != '' && $("#yizhuContentupload").length>0) {

            var yizhuContentUeditor = UE.getEditor('yizhuContentEditor');
            yizhuContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.yizhuContent != null){
                    mes = ''+ ruleForm.yizhuContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
    ...
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#yizhuContentupload").length>0) {
            var yizhuContentEditor = UE.getEditor('yizhuContentEditor');
            if (yizhuContentEditor.hasContents()) {
                $('#yizhuContent-input').attr('value', yizhuContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#yizhuName").val(ruleForm.yizhuName);
        $("#yizhuContent").val(ruleForm.yizhuContent);
