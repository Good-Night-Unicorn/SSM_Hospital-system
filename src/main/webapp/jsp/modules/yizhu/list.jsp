...
<!--/Pre Loader -->
                ...
                <div class="row breadcrumb-bar">
                    ...
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">医嘱列表</li>
                        </ol>
                    </div>
                </div>
            </div>
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        ...
                                        <div class="layui-input-inline">
                                            <input type="text" id="yizhuNameSearch" style="width: 140px;" class="form-control form-control-sm"
                                                   placeholder="医嘱名字" aria-controls="tableId">
                                        </div>
                                    </div>
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        医嘱类型
                                        <div class="layui-input-inline">
                                            <select name="yizhuTypesSelectSearch"  style="width: 150px;" id="yizhuTypesSelectSearch" class="form-control form-control-sm"
    ...
    <script>
        ...
        <!-- 级联表的级联字典表 -->
            ...
            //医嘱名字
            var yizhuNameSearchInput = $('#yizhuNameSearch');
            if( yizhuNameSearchInput != null){
                if (yizhuNameSearchInput.val() != null && yizhuNameSearchInput.val() != '') {
                    searchForm.yizhuName = $('#yizhuNameSearch').val();
                }
            }
     
                //医嘱类型
            var yizhuTypesSelectSearchInput = document.getElementById("yizhuTypesSelectSearch");
            if(yizhuTypesSelectSearchInput != null){
                var yizhuTypesIndex = yizhuTypesSelectSearchInput.selectedIndex;
                if( yizhuTypesIndex  != 0){
                    searchForm.yizhuTypes= document.getElementById("yizhuTypesSelectSearch").options[yizhuTypesIndex].value;
                }
            }
                        getDataList();
        }
        ...
        function getDataList() {
            http("yizhu/page", "GET", {
                ...
                order: sortOrder,
                //本表的
                yizhuName: searchForm.yizhuName,
                yizhuTypes: searchForm.yizhuTypes,
            //级联表的
        ...
        function setDataRow(item, number) {
            ...
            //医嘱名字
            var yizhuNameCell = document.createElement('td');
            yizhuNameCell.innerHTML = item.yizhuName;
            row.appendChild(yizhuNameCell);


            //医嘱类型
            var yizhuTypesCell = document.createElement('td');
            yizhuTypesCell.innerHTML = item.yizhuValue;
            row.appendChild(yizhuTypesCell);


                //医嘱详情
            var yizhuContentCell = document.createElement('td');
            //如果图文详情包含图片
            if (item.yizhuContent && item.yizhuContent.indexOf('img') != -1) {
                //暂时只考虑图片+文字 和 文字+图片的情况
                var beginIndex = item.yizhuContent.indexOf('<img');
                var endIndex = item.yizhuContent.indexOf('>');
                    yizhuContentCell.innerHTML = item.yizhuContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
            } else {
                if (item.yizhuContent != null && item.yizhuContent != "" && item.yizhuContent.length >= 11) {
                        yizhuContentCell.innerHTML = item.yizhuContent.substring(0, 10) + "...";
                } else {
                        yizhuContentCell.innerHTML = item.yizhuContent;
                }
            }
            row.appendChild(yizhuContentCell);
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("yizhu/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addyizhu", "addyizhu");
            window.location.href = "add-or-update.jsp"
        }
    ...
    //填充本表搜索下拉框
        ...
        function yizhuTypesSelectSearch() {
            var yizhuTypesSelectSearch = document.getElementById('yizhuTypesSelectSearch');
            if(yizhuTypesSelectSearch != null) {
                yizhuTypesSelectSearch.add(new Option('-请选择-',''));
                if (yizhuTypesOptions != null && yizhuTypesOptions.length > 0){
                    for (var i = 0; i < yizhuTypesOptions.length; i++) {
                            yizhuTypesSelectSearch.add(new Option(yizhuTypesOptions[i].indexName,yizhuTypesOptions[i].codeIndex));
                    }
                }
            }
        }
