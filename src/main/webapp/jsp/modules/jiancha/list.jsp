...
<!--/Pre Loader -->
                ...
                <div class="row breadcrumb-bar">
                    ...
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">检查列表</li>
                        </ol>
                    </div>
                </div>
            </div>
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        ...
                                        <div class="layui-input-inline">
                                            <input type="text" id="jianchaNameSearch" style="width: 140px;" class="form-control form-control-sm"
                                                   placeholder="检查名目" aria-controls="tableId">
                                        </div>
                                    </div>
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        检查类型
                                        <div class="layui-input-inline">
                                            <select name="jianchaTypesSelectSearch"  style="width: 150px;" id="jianchaTypesSelectSearch" class="form-control form-control-sm"
    ...
    <script>
        ...
        <!-- 级联表的级联字典表 -->
            ...
            //检查名目
            var jianchaNameSearchInput = $('#jianchaNameSearch');
            if( jianchaNameSearchInput != null){
                if (jianchaNameSearchInput.val() != null && jianchaNameSearchInput.val() != '') {
                    searchForm.jianchaName = $('#jianchaNameSearch').val();
                }
            }
     
                //检查类型
            var jianchaTypesSelectSearchInput = document.getElementById("jianchaTypesSelectSearch");
            if(jianchaTypesSelectSearchInput != null){
                var jianchaTypesIndex = jianchaTypesSelectSearchInput.selectedIndex;
                if( jianchaTypesIndex  != 0){
                    searchForm.jianchaTypes= document.getElementById("jianchaTypesSelectSearch").options[jianchaTypesIndex].value;
                }
            }
                        getDataList();
        }
        ...
        function getDataList() {
            http("jiancha/page", "GET", {
                page: pageIndex,
                limit: pageSize,
                sort: sortColumn,
                order: sortOrder,
                //本表的
                jianchaName: searchForm.jianchaName,
                jianchaTypes: searchForm.jianchaTypes,
            //级联表的
        ...
        function setDataRow(item, number) {
            ...
            //检查名目
            var jianchaNameCell = document.createElement('td');
            jianchaNameCell.innerHTML = item.jianchaName;
            row.appendChild(jianchaNameCell);


            //检查类型
            var jianchaTypesCell = document.createElement('td');
            jianchaTypesCell.innerHTML = item.jianchaValue;
            row.appendChild(jianchaTypesCell);


                //检查结果
            var jianchaContentCell = document.createElement('td');
            //如果图文详情包含图片
            if (item.jianchaContent && item.jianchaContent.indexOf('img') != -1) {
                //暂时只考虑图片+文字 和 文字+图片的情况
                var beginIndex = item.jianchaContent.indexOf('<img');
                var endIndex = item.jianchaContent.indexOf('>');
                    jianchaContentCell.innerHTML = item.jianchaContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
            } else {
                if (item.jianchaContent != null && item.jianchaContent != "" && item.jianchaContent.length >= 11) {
                        jianchaContentCell.innerHTML = item.jianchaContent.substring(0, 10) + "...";
                } else {
                        jianchaContentCell.innerHTML = item.jianchaContent;
                }
            }
            row.appendChild(jianchaContentCell);
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("jiancha/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addjiancha", "addjiancha");
            window.location.href = "add-or-update.jsp"
        }
    ...
    //填充本表搜索下拉框
        ...
        function jianchaTypesSelectSearch() {
            var jianchaTypesSelectSearch = document.getElementById('jianchaTypesSelectSearch');
            if(jianchaTypesSelectSearch != null) {
                jianchaTypesSelectSearch.add(new Option('-请选择-',''));
                if (jianchaTypesOptions != null && jianchaTypesOptions.length > 0){
                    for (var i = 0; i < jianchaTypesOptions.length; i++) {
                            jianchaTypesSelectSearch.add(new Option(jianchaTypesOptions[i].indexName,jianchaTypesOptions[i].codeIndex));
                    }
                }
            }
        }
