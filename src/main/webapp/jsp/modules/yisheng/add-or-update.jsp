                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input id="yishengIdNumber" name="yishengIdNumber" class="form-control"
                                               onchange="yishengIdNumberChickValue(this)"  placeholder="医生身份证号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>医生照片</label>
                                        <img id="yishengPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="yishengPhotoupload"
                                               class="form-control-file">
                                        <input name="yishengPhoto" id="yishengPhoto-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>性别</label>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#yishengPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('yishengPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('yishengPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    //数字检查
        ...
        function yishengIdNumberChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                username: "required",
                password: "required",
                yishengName: "required",
                yishengPhone: "required",
                yishengIdNumber: "required",
                yishengPhoto: "required",
                sexTypes: "required",
            },
            messages: {
                username: "账户不能为空",
                password: "密码不能为空",
                yishengName: "医生姓名不能为空",
                yishengPhone: "医生手机号不能为空",
                yishengIdNumber: "医生身份证号不能为空",
                yishengPhoto: "医生照片不能为空",
                sexTypes: "性别不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addyisheng = window.sessionStorage.getItem("addyisheng");
        if (addyisheng != null && addyisheng != "" && addyisheng != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("yisheng/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
